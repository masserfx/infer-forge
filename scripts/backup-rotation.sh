#!/usr/bin/env bash
# Backup rotation script for inferbox
...
DAILY_BACKUPS=$(find "$BACKUP_DIR" -name "inferbox-backup-*.sql.gz" -type f -mtime -7 | wc -l)
DAILY_TO_DELETE=$(find "$BACKUP_DIR" -name "inferbox-backup-*.sql.gz" -type f -mtime +7)
...
WEEKLY_BACKUPS=$(find "$BACKUP_DIR" -name "inferbox-weekly-*.sql.gz" -type f -mtime -28 | wc -l)
WEEKLY_TO_DELETE=$(find "$BACKUP_DIR" -name "inferbox-weekly-*.sql.gz" -type f -mtime +28)
...
MONTHLY_TO_DELETE=$(find "$BACKUP_DIR" -name "inferbox-weekly-*.sql.gz" -type f -mtime +90)
...
DAILY_COUNT=$(find "$BACKUP_DIR" -name "inferbox-backup-*.sql.gz" -type f | wc -l)
WEEKLY_COUNT=$(find "$BACKUP_DIR" -name "inferbox-weekly-*.sql.gz" -type f | wc -l)
